...
func newSegmentGroup(logger logrus.FieldLogger,
	...
	for _, entry := range list {
		...
		segment, err := newSegment(filepath.Join(sg.dir, entry.Name()), logger, sg.makeExistsOnLower(segmentIndex),
...
func (sg *SegmentGroup) add(path string) error {
	...
	segment, err := newSegment(path, sg.logger, sg.makeExistsOnLower(newSegmentIndex),
		sg.mmapContents, sg.useBloomFilter, sg.calcCountNetAdditions)
	if err != nil {
